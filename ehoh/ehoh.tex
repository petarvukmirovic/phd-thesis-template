...
good performance. It gets its ``brainiac'' name by emphasizing 
intelligent heuristics more than raw speed. E regularly scores among the top
...
and its semantics by interpreting the Boolean type $o$ as a two-element domain. Functional extensionality, the property that two functions are equal if they
...
%
In particular, when $x()$'s \verb|binding| field is updated, this does not affect the \verb|binding| of $x(\cst{b})$. 
...
reason, in this chapter, we focus on KBO.
...
and $\cst{f} \; \tuple{t}{n}$, because of partial application it is possible that $m \not= n$; 
this required changing the implementation to
...
%      \smallskip
  ...
  %
  An algorithm which closely follows the Ehoh implementation, abstracting away from
  complications such as prefix optimization is presented below. We assume a flattened
...
nullary symbols \cite{bn-98-tr-and-all-that}, but E implements matching
separately.
...
\looseness=-1
When generalizing the data structure to \lfhol, we face a complication due to
partial application. First-order terms can only be stored in leaf nodes, but in
Ehoh we must also be able to represent partially applied terms, such as
$\cst{f}$, $\cst{g}$, or $\cst{g}\;\cst{a}$ (assuming, as above, that $\cst{f}$
is unary and $\cst{g}$ is binary). Conceptually, this can be solved by storing a
Boolean on each node indicating whether it is an accepting state (i.e., if it
corresponds to a term in the indexed set). In the implementation, the change is
more subtle, because several parts of E's code implicitly assume that only leaf
nodes are accepting.
...
\begin{quotex}
  ...
  possibly visiting the child node labeled with a function symbol. The children of a node can be iterated through using a function
...
Instead of creating and storing substitutions explicitly, the algorithm simply remembers
whether the variable was bound in this step or not, in the
$\mathit{var\_unbound}$ tuple component. Then it relies on the label~$x$ of the
current node and its \textit{binding} field to carry the substitutions.
Similarly, since the strategy is to traverse the tree by first visiting the
variable-labeled child nodes, it needs to remember how far it has come with
this traversal. This information is stored in the $\mathit{c\_iter}$ tuple
component.
...
incomplete rules my colleagues and me had experimented with in the context of Zipperposition.
...
of variables and symbols that are ignored by the heuristics.
...
The selection function E uses most often is probably
...
\begin{rep}
  ...
  inference partners, a behavior that can lead to creating many unnecessary
  clauses and should thus be avoided. On the other hand, shorter proofs might be
  found if we prefer selecting applied variables. To cover both scenarios, we
  implemented selection functions that prefer or defer selecting applied
  variables. % and that break ties using \texttt{SelectMLCAPP}.
  ...
  within a partition.
  E's main loop visits a set of priority queues in round-robin fashion. From a given
  ...
  fairness (i.e., that each nonredundant clause is eventually chosen).
  \end{rep}
...
mode when running on TPTP benchmarks. We call it \emph{boa} (``best of \emph{auto}'')\begin{conf}.\end{conf}\begin{rep}:
%
...
Skolem symbols are always applied to a minimal number of arguments.
...
Appropriately chosen term order will result in a similar outcome without
removing the defined symbol. We will enable definition unfolding again once we
have added support for $\lambda$-terms (Chapter \ref{ch:ehoh2}).
...
unsoundness. We needed to undo this in Ehoh. Ehoh also removes trivial
...
To answer the first question, we evaluated each new heuristic scheme independently. From
...
\begin{rep}This gave an idea of how each parameter affected overall
performance.\end{rep}
...
Clause selection is the heuristic component that we extended the most. We had to
...
\emph{boa}'s queue that uses
\texttt{4.CRS\-Weight(}\ldots\texttt{)}, with the queue ordered by
\texttt{4.CRSTypeWeight(}\ldots\texttt{)}. The original
heuristic is called \texttt{W} and the type-aware alternative \texttt{TW}.
%
We intuitively chose nine values for testing the effect of the applied
...
the advantage that they can instantiate higher-order
...
We ran all provers except Satallax (which supports only THF) on first-order
...
\begin{rep}
  ...
  The work described in this chapter was large in scope: it required modifying almost all parts of the E
  prover, from the parser, to the inference engine, to heuristics. 
  The invariant that variables cannot be applied and that symbols are always
  ...
  we found the generalization manageable. The generalization put us in a position to add
  ...
  the first-order prover Otter\begin{rep} \cite{mcc-03-otter}\end{rep}.
  % The approach is pragmatic. , with little emphasis on completeness.
  Zipperposition, based on superposition, was extended
  ...
  perform very well compared with native provers
  ...
  the translation gap. The applicative encoding buries the \lfhol{} terms' heads under
...
reasoning had not assimilated some of the most successful methods for
...
It was heavily inspired by the techniques implemented in Zipperposition, which helped it
dominate CASC in 2020 and 2021.
\end{sloppypar}
