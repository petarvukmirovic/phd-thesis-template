...
presented in Sect.~\ref{sec:unif:solid-oracle}.
%
...
Last, the JP procedure does not allow applying substitutions and
$\beta$-reducing lazily.
The rules of simpler procedures (e.g., first-order
\cite{hv-09-unifalgs} and pattern unification \cite{tn-93-patterns}) depend on only
the heads of the unification pair.
Thus, to
determine the next step, implementations of these procedures need to
substitute and
$\beta$-reduce until only the heads of the current unification
...
Unless otherwise stated, we take the unification constraint $s \unif t$ to be an
...
The procedure requires an infinite supply of fresh free variables that must be
disjoint from the variables occurring in the initial multiset $E$. Whenever a
transition $(E,\sigma) \newunifarrow (E',\sigma')$ is made, all fresh variables
used in $\sigma'$ are removed from the supply and cannot be used again as fresh
variables.
...
\begin{description}[labelwidth=\widthof{\rm\textsf{Normalize$_{\alpha\eta}$}}]
  ...
  \item[\rm\unifrulename{Normalize$_{\alpha\eta}$}]
      ...
      $\overline{z}_m$ are bound variables fresh with respect to $s$ and $t$, $s' = \{x_1 \mapsto z_1, \ldots, x_m \mapsto z_m \}\,s$,
      and $t' = \{y_1 \mapsto z_1, \ldots, y_n \mapsto z_n \}\,t$
  % \smallskip
...
The forms of the rules \unifrulename{OracleSucc} and \unifrulename{Bind} are similar: both
...
explore the whole search space for any given constraint. To compute a CSU using efficient oracles and to discover
...
substitution just enough to ensure that the heads are the ones that would be obtained if
...
constraint is inside the fragment, the  substitution must be fully applied and
the constraint must be $\beta$-normalized.
...
Most oracles contain a decomposition operation that requires only a partial application of
...
This allows detecting that the problem is not in the fragment without fully applying
...
Our procedure chooses child nodes based on only the heads of the
...
fashion. Moreover, as the JP procedure does not feature a rule similar to
...
Finally, a measure based on values of $E_j$ and $\varrho_j$ that decreases with 
each application of the rules is employed. Therefore, eventually, the target substitution
$\sigma$ will be reached.
...
unify $s_{i} \unif t_{i}$. For example, $\{X \mapsto \lam{\overline{x}}{\cst{a}}\}$ is a unifier that does
  ...
  \end{exa}
  Definition \ref{unif:def:omega-simple} is slightly too restrictive for our purposes.
...
Prehofer focused on the preunification problem, remarking that the resulting
...
% Maybe it should also use the same names for substitutions as in the previous para?
 \begin{exa}
  ...
  lead to a nonunifiable problem, so the imitation of~$\cst{g}$ is performed, yielding a
  ...
  \cst{a}\}$. Again, $\cst{a}$ can only be imitated for $F_1$---building a new 
  binding $\sigma_2 = \{F_1 \mapsto \lambda x.\, \cst{a} \}$. Finally, this
  yields the problem $\{ F_2 \, (\cst{f} \, \cst{a}) \unif G \, \cst{a} \}$.
  According to Lemma \ref{unif:lemma:solid-vars-diff-mgu}, CSUs for the
  problems $J_1 \, \cst{a} = \cst{f} \, \cst{a}$ and $I_1 \, (\cst{f} \, \cst{a})
  \unif \cst{a}$ are found using PT. The latter problem has a singleton CSU $\{I_1 \mapsto \lambda x.\,
  \cst{a}\}$, whereas the former has a CSU containing $\{ J_1 \mapsto \lambda x.\,
  \cst{f} \, x \}$ and $\{ J_1 \mapsto \lambda x.\, \cst{f} \, \cst{a} \}$.
  Combining these solutions, an MGU $\sigma_3 =
  \{ F_2 \mapsto \lambda x.\, H \, x \, x \, \cst{a},\,
  G   \mapsto \lambda x.\, H \, (\cst{f} \, \cst{a}) \, (\cst{f} \, x) \, x \}$ 
  is obtained for $F_2 \, (\cst{f}\, \cst{a}) \unif G \, \cst{a}$.
  %\begin{equation*}
  %    F_2 \mapsto \lambda x.\, H \, x \, x \, \cst{a} \\
  %    G   \mapsto \lambda x.\, H \, (\cst{f} \, \cst{a}) \, (\cst{f} \, x) \, x
  %\end{equation*}
  Finally, the MGU of the original problem is computed: $\sigma = \sigma_3 \sigma_2 \sigma_1 = \{ F
...
and admit only infinite CSUs can easily be found. The problem $\{ \lambda x.\, F \, (\cst{f} \, x) \unif \lambda x.\, \cst{f} \,
...
procedure is implemented in a module that is parameterized by another module
providing oracles and implementing the \textsf{Bind} step. In this way we
...
from a particular lazy list, the procedure periodically returns an empty
...
well-performing configuration we used for the 2019 installment of THF
division of the CASC theorem proving competition \cite{gs-19-casc27}. The cv
...
and identifications, respectively. The values for $a,b,c,d,$ and $e$ are chosen following
the intuition that functional projection and identification are the most explosive
bindings.  Figure~\ref{fig:oracles} summarizes the
