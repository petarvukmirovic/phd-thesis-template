...
\cst{b}_1 \, \cst{b}_2 \not\eq \itrue \llor P \, \cst{c}_1 \, \cst{c}_2 \not\eq
\itrue$, we can apply \infname{ElimLeibniz$+$} using the substitution $\sigma = \{ P \mapsto \lam{xy}{x \eq \cst{b}_1} \}$
...
\infname{BoolER}, $\sigma$ is a unifier of $ s $ and $ \neg s' $. In the $+-$
...
either side of a literal. Before applying the \infname{DC} rule on a
...
Before the number of occurrences of $f$ is checked, we first check (using a fast, incomplete matching algorithm) if there is a
...
It enables \infname{BoolSimp} rule,
\infname{EC} rule, \infname{PI} rule in \emph{Pragmatic} mode with $k=2$,
\infname{ElimLeibniz} and \infname{ElimPredVar} rules, \infname{BoolER} rule, and \infname{BoolEF} rules. 
...
swaps this rule for \infname{Cases}. Although the margin is slim, the results show it is usually preferable to select leftmost-outermost
subterm.
...
no other mode (including disabling \infname{PI} rule) can. Modes
...
\begin{figure}[t]
  \begin{center}
    ...
    \end{tabular}}
    \caption{Effect of \infname{PI} rule on success rate}
    \label{fig:pi}
  \end{center}
\end{figure}
\begin{figure}[t]
...
Similarly, Figure \ref{fig:bool-rules} discusses merits of including ($+$) or
...
\looseness=-1
Combining these two rules with dynamic clausification is more useful: when the rule
\infname{IC} is replaced by the rule \infname{DC}, the success rate increases
...
\begin{figure}[t]
  ...
  \begin{center}
    ...
    \relax{\begin{tabular}{@{}l@{\hskip 2em}c@{\hskip 2em}c@{\hskip 2em}c@{\hskip 2em}c@{\hskip 2em}c}
      ...
      \MIDLINE
      pure    & 1806 (5) &  1627 (0) & 2067 (0)            & 1924 (7) & 1980 (\phantom{0}0)                \\
