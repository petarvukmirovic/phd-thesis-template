...
approaches such as the CDCL calculus \cite{mss-96-cdcl}. Crucial improvement of
...
\upsilon$ where both $\tau$ and $\upsilon$ are types. When we write $\tau_1
\rightarrow \cdots \rightarrow \tau_n \rightarrow \upsilon$ we assume $\upsilon$
to be a base type ($\upsilon \in T$), by convention. Each symbol $\cst{f} \in
...
variables gives rise to \emph{loose bound variables} (e.g., $y$ in $\lamx{y \,
\cst{a}}$) \cite{tn-93-patterns}. Note that there is no requirement that a
...
call terms of Boolean type \emph{formulas}. Our logic also supports Hilbert
...
\lambda y. \, \{x \mapsto y\}(s)$ where $y$ does not occur loosely bound in $s$
...
condition that $x$ is not loosely bound in $x$. We write $s \equi t$ if terms
...
implicitly universally quantified. Note that $\llor$ denoting disjunction in
clauses is different from the formula-building $\ior$. In general,
formula-building logical connectives are written in boldface. In standard,
...
$\neg\,\cst{even}(x)$ is encoded as $\neglit{\cst{even}(x)}$. To lighten the
notation, we sometimes write the predicate literals in non-encoded form.
...
clauses. As this process is quite prolific it also uses the term order to determine
...
inferences are computed in a systematic, fair manner, empty clause ($\bot$) will
...
mathematicians do when they ``replace equals by equals''. Name paramodulation is
also used for rules that replace equals by equals in calculi preceding superposition.  The left premise intuitively
states that an equation $s \eq t$ holds under condition $C$ (or more precisely,
its negation). Similarly, the right premise asserts that a (dis)equation holds
...
With \emph{proof state} we assume the pair $(\mathcal{P}, \mathcal{A})$.
...
theorem provers: E \cite{ss-02-brainiac} and Zipperposition
\cite{sc-15-simon-phd,bbtvw-21-sup-lam}. The latter was used as a testbed for
prototyping ideas, while the former is the main target of our work as it is more
efficient.
