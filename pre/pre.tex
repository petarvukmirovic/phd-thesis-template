...
We distinguish a set of base types $T$ which is required to include the Boolean
...
$\ourmodel~\models_\xi~N$ to denote that a model $\ourmodel$ satisfies a formula
...
$\cst{f}(\tuplen{t})$ at position $i.p$ (with $.$ denoting prepending operation). A {\em context} is a term with zero or
...
replacing all variables of $t$ by the corresponding values of the mapping $\sigma$.
...
$\{ X \mapsto Y\}$ or  $\{ Y \mapsto X \}$. First-order logic also admits efficient, linear-time algorithms for computing MGUs \cite{hv-09-unifalgs}.
...
many unnecessary clauses might be created. To identify unnecessary clauses in the search
...
rules replace the pre\-mises with conclusions, and we denote them
...
\namedsimp{RN}{s \eq t \qquad u \noteq v \llor D}
              {s \eq t \qquad u[p \leftarrow \sigma(t)] \noteq v  \llor D}
\]
...
extend an efficient superposition prover to higher-order logic. My colleagues
and me did the necessary theoretical work by extending the superposition calculus
using the same roadmap:
...
\end{enumerate}
%
Note that in this thesis the name $\lambda$-superposition is used for both \lsup{}
and \osup{}.
...
In the last decade, together with its derivatives, it was usually the
