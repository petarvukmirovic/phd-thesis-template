...
\begin{abstract}
    ...
    description of the superposition calculus. The text of this chapter is
...
$\left(\varrho\sigma\right)t=\varrho\left(\sigma t\right)$. We say that substitution is grounding
if it maps all variables to ground terms.
...
as $\sigmacl{l}$ and $\sigmacl{C}$. We say $\sigmacl{C}$ is a ground instance of $C$ is $\sigma$ is a grounding
substitution.
...
Literal selection function is a function that selects a (multi)subset of literals.
...
unordered pair of two terms of the same type. The most general unifier is a
...
denoting iterated, $i$-fold application of $\cst{f}$. To generalize most general
unifiers to higher-order logic, the concept of complete sets of unifiers was
introduced.
...
corresponds is a one-element CSU. A unifier of terms $s$ and $t$ is a unifier of
...
unifiers as first-order or higher-order and let the context provide enough
information.
...
With $t|_p$ we denote subterm of $t$ at position $p$ and with $t[p \rightarrow
t']$ replacement of this subterm with $t'$. Let \selfun be a selection function
and $\succ$ a reduction ordering, $C = l \llor C'$ a clause, and $\sigma$ a
substitution. We say that \sigmacl{l} is eligible for resolution if: (1) nothing
is selected by \selfun and \sigmacl{l} is $\succ$-maximal within literals in
...
it still grows rather fast. To identify unnecessary clauses from the search
space, superposition features \emph{redundancy criterion}
\cite[Sect.~4.2.2]{bg-01-resolution}. We say that a ground clause $C$ is
redundant in a ground set of clauses $N$ if it is entailed by a subset of
...
\[
\namedinference{RN}{s \eq t \qquad u \noteq v \llor D}{(u[p \leftarrow \sigma(t)] \eq v \llor C \llor D)\sigma}
\]
where $\sigma = \mgu(u|_p, s)$, and $\sigma(s) \succ \sigma(t)$.

Simple rules such as removing duplicate literals, literals of the form $s \noteq
t$ or tautological clauses are clearly justified by the redundancy criterion.
Schulz gives an extensive list of such rules\cite{ss-02-brainiac}.
...
The provers we discuss in this thesis are both DISCOUNT-loop provers.
