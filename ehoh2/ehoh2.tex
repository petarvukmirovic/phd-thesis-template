...
\begin{abstract}%
  ...
  we propose to extend a first-order prover with native support for
  higher-order features. Building on our extension of E to $\lambda$-free
  higher-order logic, we extended E to full higher-order logic.
...
extension gave us a feeling for the difficulties that might be encountered on the
way to full higher-order logic. In the chapters that precede this one,
we discussed many ways in which those difficulties can be overcome.
In this chapter, we fulfill the promise given in the beginning of the thesis: We present the extension of
Ehoh to full higher-order logic using incomplete variants
of $\lambda$-superposition. This prover is called \ehohii.
...
this experience to choose a set of effective rules that could easily be
...
problems and found that no overhead has been introduced by the
extension to higher-order logic.

\ourpara{Background} The logic that \ehohii{} targets is the monomorphic higher-order
logic described in Sect.~\ref{sec:pre:hol}. We reuse all the notions from this
section, with the corresponding notations. As in the previous chapter, we
...
higher-order logic, Ehoh's $\lambda$-free cell data structure had to
...
A term is $\beta$-reduced as follows: When a cell of the
...
Second, in line with the gracefulness principle, we wanted \ehohii{} to incur
...
Then the $k = \min\{j,l,n\}$ rightmost outermost $\lambda$ binders in $\internallam(\db{0},
\ldots, \internallam(\db{0}, t) \ldots)$ can be removed and $t$ can be replaced %the body
by the variant of
...
\[\begin{array}{@{}l@{\kern1em}l@{\kern1em}l@{\kern1em}l@{\kern1em}l@{}}
  ...
  \begin{sloppypar}
    ...
    that simulate basic superposition calculus rules,
  ...
  and \NumberOK{7\%} more problems on SH benchmarks, where Ehoh was already very
  successful.
  ...
  \NumberOK{181} SH problems and \NumberOK{24} TPTP problems that Zipperposition-coop
  ...
  both in this first-order (FO) mode and in higher-order (HO) mode using 30 s CPU and wallclock timeout.
