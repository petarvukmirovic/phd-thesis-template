...
\begin{itemize}
  ...
  colleagues. Developing this calculus helped us understand in which
  ...
  we further extended Ehoh to full higher-order logic, obtaining prover we call
  \newehoh{}. We again modified term structure, implemented new unification and
  ...
  to simplify problems in propositional logic, a simpler logic that disallows quantification,
...
Earliest attempts to automate higher-order logic can be traced back to
...
calculi that are directly applied on higher-order formulas.

These attempts resulted in early higher-order theorem provers. Andrews and
collegaues developed TPS~\cite{abinpx-96-tps}: a semi-automatic prover which
...
\cite{sb-21-leo3} is based on higher-order analytic tableu and uses SAT solver
to close the branches. It won of higher-order division of CASC on 7 ocassions:
in 2011 and every year from 2013 to 2019 (and once in 2011).

Various reserachers also experimented with the idea of extending a first-order
prover to higher-order logic. Beeson \cite{mb-04-lam-logic} implemented second
order unification and added $\lambda$-terms to one of the best superposition
provers of the time, Otter~\cite{mcc-03-otter}. Cruanes extended Zipperposition
...
prover. Jensen and Pietrzykowski introduced a complete algorithm for enumerating
elements unifiers for higher-order terms \cite{jp-76-unif}. Huet noticed that
some calculi remaing complete if solving a difficult subprobem of higher-order
unification (`flex-flex'' pairs) is delayed. He described algorithm to solve
this easier problem in a more efficient way \cite{gh-75-unification}. Dougherty
desgined an algorithm to enumerate higher-order unifiers using higher-order
